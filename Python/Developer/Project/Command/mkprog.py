...
log_path = f"{dir_prog}\\Python\\Developer\\Project\\Command\\logs\\{filename}.log"
...
def configure_dirprogramming_file(directory_name, short_name, venv, isverbose):
    prog = f"{dir_prog}\\Bash\\Developer\\Project\\Command\\dirprogramming"
    script_list = []
    ...
    script_list.append('\n')
    with open(prog, 'w') as file:
        for word in "\n".join(script_list):
            file.write(word)
        file.close()
    print_info(f"'{prog}' has modified ({directory_name})", isverbose)
    logging.info(f"'{prog}' has modified ({directory_name})")
